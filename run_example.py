...
for idf, filename in enumerate(glob.glob(os.path.join(inpath, '*.arff'))):
    ...
    for idx in range(nrep):
        ...
        plt.scatter(data[training_len:,0],data[training_len:,1], s=1, cmap='coolwarm', c=scores_test, rasterized=True)
        ...
        plt.scatter(data[training_len:,0],data[training_len:,1], s=1, cmap='Reds', c=labels_test, rasterized=True)
...
ax = plt.gca()
plt.scatter(data[0:training_len,0],data[0:training_len,1], s=1, cmap='coolwarm', c=labels[0:training_len], rasterized=True)
...
ax = plt.gca()
plt.scatter(data[training_len:,0],data[training_len:,1], s=1, cmap='Greys', c=0.5*np.ones([training_len]), rasterized=True)
#plt.colorbar(ticks=[0,1])
plt.scatter(data[training_len:,0],data[training_len:,1], s=1, cmap='Greys', c='black', rasterized=True)
ax.set_xlim((-0.05, 1.01))
ax.set_ylim((-0.05, 1.06))
